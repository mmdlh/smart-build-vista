...
function DataTable({ title, headers, rows }: { title: string; headers: string[]; rows: string[][] }) {
  return <Panel title={title} meta={`共 ${rows.length} 条`}><div className="overflow-x-auto"><table className="w-full min-w-[560px] text-left"><thead><tr>{headers.map(h => <th key={h} className="border-b border-border px-3 py-2 text-[10px] font-medium text-muted-foreground">{h}</th>)}</tr></thead><tbody>{rows.map((r, i) => <tr key={i} className="table-row">{r.map((v, j) => <td key={j} className="border-b border-border/50 px-3 py-3 text-xs text-foreground">{j === r.length - 1 ? <span className="inline-flex items-center gap-1.5 text-success"><StatusDot status={v.includes("异常") || v.includes("逾期") ? "danger" : "ok"} />{v}</span> : v}</td>)}</tr>)}</tbody></table></div></Panel>;
}
...
function Green() {
  const env = trend.map((x,i)=>({...x, pm25:[27,31,38,43,34,29][i], pm10:[52,56,68,75,61,54][i], tsp:[71,78,91,106,89,76][i]}));
